...
export const getContractionSummary = (
  ...
  return {
    ...
    averageDuration: meanBy(contractions, (c) =>
      differenceInSeconds(c.end, c.start)
    ),
    averageFrequency: mean(frequencies),
    averagePain: meanBy(contractions, (c) => c.pain),
    range:
      isNil(latest) || isNil(earliest)
        ? 0
        : differenceInSeconds(latest.end, earliest.start),
  };
};

export const isHospitalTime = (summary: ContractionSummary): boolean => {
  return (
    summary.averageDuration >= HOSPITAL_LIMITS.DURATION &&
...
const TOO_LONG_ENOUGH_CONTRACTIONS: Contraction[] = PERFECT_CONTRACTIONS.map(
  (c) => {
    return {
      ...c,
...
const NOT_LONG_ENOUGH_CONTRACTIONS: Contraction[] = PERFECT_CONTRACTIONS.map(
  (c) => {
    return {
      ...c,
